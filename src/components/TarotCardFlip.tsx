...
export const TarotCardFlip = React.memo(({
  ...
  useEffect(() => {
    if (externalFlipped !== undefined && externalFlipped !== revealed) {
      if (externalFlipped) {
        triggerFlip();
      } else {
        // flip back
  ...
  const triggerFlip = useCallback(() => {
    // Phase 1: rotate to 90° (back disappears)
    rotateY.value = withTiming(90, { duration: 300, easing: Easing.in(Easing.quad) }, () => {
      // At 90°: swap face — do this via runOnJS
      runOnJS(setRevealed)(true);
      runOnJS(fireHaptics)();
      // Phase 2: spring from 90° back to 0° (front appears with overshoot)
  ...
  // ── Animated styles ──────────────────────────────────────────────────────────
  // Front face (back of card = sacred geometry)
  const backFaceStyle = useAnimatedStyle(() => {
    const rot = interpolate(rotateY.value, [0, 90], [0, 90]);
    return {
      transform: [{ perspective: 800 }, { rotateY: `${rot}deg` }],
      backfaceVisibility: 'hidden',
      opacity: rotateY.value >= 90 ? 0 : 1,
    };
  });

  // Revealed face (front of card = card content)
  const frontFaceStyle = useAnimatedStyle(() => {
    // front face starts flipped at 180° and arrives at 0°
    const rot = interpolate(rotateY.value, [0, 90], [-90, 0]);
    return {
      transform: [{ perspective: 800 }, { rotateY: `${rot}deg` }],
      backfaceVisibility: 'hidden',
      opacity: rotateY.value >= 90 ? 0 : 1,
        ...
        >
          ...
          <View
            style={{
              ...
              backgroundColor: isLight ? 'rgba(0,0,0,0.08)' : 'rgba(0,0,0,0.22)',
              borderRadius: 16,
            }}
          />
          ...
          {isReversed && (
            <View style={{
              ...
              <Text style={{ fontSize: 10, fontWeight: '700', letterSpacing: 1.5, color: '#FFF' }}>
                ODWRÓCONA
              </Text>
            </View>
          )}
          ...
          {/* Holographic shimmer overlay (pan gesture) */}
