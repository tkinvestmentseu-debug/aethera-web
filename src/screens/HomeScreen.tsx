  ...
  if (id === 'aura') return (
    <>
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
        ...
        <ActionTile icon={Eye} label={tr('home.aura.palmReading', 'Czytanie z Dłoni', 'Palm Reading')} sublabel={tr('home.aura.palmReadingSub', 'Linie życia, serca i rozumu', 'Life, heart, and mind lines')} accent={ac} onPress={() => navigation.navigate('PalmReading')} delay={60} isLight={isLight} />
        <ActionTile icon={Target} label={tr('home.aura.visionBoard', 'Tablica Wizji', 'Vision Board')} sublabel={tr('home.aura.visionBoardSub', 'Przyciągaj przez pole energii', 'Attract through your energy field')} accent={ac} onPress={() => navigation.navigate('VisionBoard')} delay={120} isLight={isLight} />
        <ActionTile icon={Sparkles} label={tr('home.aura.mantra', 'Generator Mantry', 'Mantra Generator')} sublabel={tr('home.aura.mantraSub', 'Mantra rezonująca z Twoją aurą', 'A mantra resonating with your aura')} accent={ac} onPress={() => navigation.navigate('MantraGenerator')} delay={180} isLight={isLight} />
      </View>
      ...
      <ExploreRow title={tr('home.aura.numerology', 'Numerologia', 'Numerology')} desc={tr('home.aura.numerologyDesc', 'Liczby jako klucz do wibracji i energii Twojej duszy.', 'Numbers as the key to the vibration and energy of your soul.')} accent={ac} onPress={() => navigation.navigate('Numerology')} delay={40} isLight={isLight} />
      <ExploreRow title={tr('home.aura.runes', 'Rzut Runami', 'Rune Cast')} desc={tr('home.aura.runesDesc', 'Pradawne symbole ujawniają stan i kierunek Twojego pola energetycznego.', 'Ancient symbols reveal the state and direction of your energy field.')} accent={ac} onPress={() => navigation.navigate('RuneCast')} delay={80} isLight={isLight} />
      ...
      <ExploreRow title={tr('home.aura.herbalAlchemy', 'Alchemia Ziołowa', 'Herbal Alchemy')} desc={tr('home.aura.herbalAlchemyDesc', 'Zioła i esencje naturalne wspierające oraz oczyszczające aurę.', 'Natural herbs and essences that support and cleanse the aura.')} accent={ac} onPress={() => navigation.navigate('HerbalAlchemy')} delay={160} isLight={isLight} />
      <ExploreRow title={tr('home.aura.matrix', 'Matryca Przeznaczenia', 'Matrix of Fate')} desc={tr('home.aura.matrixDesc', 'Kwadrat Pitagorasa odkrywa wzorce Twojej duszy i karmiczne pole energii.', 'The Pythagorean square reveals your soul patterns and karmic energy field.')} accent={ac} onPress={() => navigation.navigate('Matrix')} delay={200} isLight={isLight} />
    </>
  );
